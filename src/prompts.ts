...
/**
 * Marker comment to identify agent-foreman managed section
 */
const HARNESS_SECTION_START = "<!-- agent-foreman:start -->";
const HARNESS_SECTION_END = "<!-- agent-foreman:end -->";

/**
 * Generate the Long-Task Harness section content
 */
export function generateHarnessSection(goal: string): string {
  return `${HARNESS_SECTION_START}
## Long-Task Harness
...
/**
 * Merge harness section into existing CLAUDE.md content
 * If existing content has agent-foreman section, replace it
 * Otherwise append at the end
 */
export function mergeClaudeMd(existingContent: string, goal: string): string {
  const harnessSection = generateHarnessSection(goal);

  // Check if existing content has agent-foreman markers
  const startIdx = existingContent.indexOf(HARNESS_SECTION_START);
  const endIdx = existingContent.indexOf(HARNESS_SECTION_END);

  if (startIdx !== -1 && endIdx !== -1) {
    // Replace existing section
    const before = existingContent.substring(0, startIdx);
    const after = existingContent.substring(endIdx + HARNESS_SECTION_END.length);
    return before.trimEnd() + "\n\n" + harnessSection + after.trimStart();
  }
