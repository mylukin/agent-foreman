...
async function main() {
  ...
  await yargs(hideBin(process.argv))
    ...
    .command(
      "analyze <spec_path>",
      "Analyze a requirement document and generate implementation steps as JSON files",
      (yargs) =>
        yargs.positional("spec_path", {
          describe: "Path to the requirement spec file (text/markdown)",
          type: "string",
          demandOption: true,
        }),
      async (argv) => {
        await runAnalyze(argv.spec_path as string);
      }
    )
    .command(
      "run <steps_dir>",
      "Execute implementation steps generated by analyze command",
      (yargs) =>
        yargs.positional("steps_dir", {
          describe: "Directory containing step JSON files (e.g. <需求名字>需求实现步骤)",
          type: "string",
          demandOption: true,
        }),
      async (argv) => {
...
/**
 * Analyze a requirement spec and generate step JSON files
 */
async function runAnalyze(specPath: string) {
  ...
  const outputDir = path.join(cwd, safeBaseName);

  // Ensure target directory does not already exist
  try {
    await fs.access(outputDir);
    console.log(chalk.red(`✗ Target directory already exists: ${safeBaseName}`));
    process.exit(1);
  } catch {
  ...
  for (let index = 0; index < steps.length; index++) {
    ...
    const baseSlugSource = step.slug || step.description || `step-${stepIndex}`;
    const normalizedSlug = slugify(baseSlugSource);

    if (usedSlugs.has(normalizedSlug)) {
      console.log(
        chalk.red(
          `✗ Duplicate slug detected after normalization: "${normalizedSlug}". Please re-run analyze or adjust spec.`
        )
      );
      process.exit(1);
    }
    ...
    const fileName = `${stepIndex}-${normalizedSlug}.json`;
