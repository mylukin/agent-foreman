...
目标不是做一个"万能 Coding 智能体"，而是给 **你 + 任意 LLM** 提供一个：
...
* `blocked` 是用于告诉智能体：别在这个上面浪费时间了，先甘当 backlog。
...
* 将来加 e2e（Playwright / Cypress）时：
  ...
  * 可以在 Feature 里加一个 `e2eTestId` 字段，绑定测试用例名，方便智能体自动调用
