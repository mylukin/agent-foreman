/**
 * Integration tests for end-to-end analyze → run flow
 *
 ...
 * - Status values flow from 🔴 待完成 → 🟡 进行中 → 🟢 已完成 and are reflected
...
async function createFakeAnalyzeAndRunAgent(binDir: string): Promise<string> {
  ...
  const scriptContent = [
    ...
    "  echo '{\"requirementName\":\"Analyze+Run 测试需求\"}'",
    "fi",
    "",
...
describe("CLI analyze + run flow (integration)", () => {
  ...
  testOrSkip("[integration] analyze output can be consumed by run end-to-end", async () => {
    ...
    await fs.writeFile(
      specPath,
      "# Analyze + Run 测试需求\n\n这是一个用于验证 analyze 与 run 端到端流程的最小需求文档。",
      "utf-8",
    );
    ...
    // Discover the generated steps directory (<需求名字>需求实现步骤)
    ...
    expect(firstStep.status).toBe("🔴 待完成");
    ...
    for (const file of finalStepFiles) {
      ...
      expect(content.status).toBe("🟢 已完成");
    }
    ...
    for (const file of finalStepFiles) {
      ...
      // 执行前状态列应包含待完成，执行后状态列应包含已完成，结果列应标记为成功
      expect(row as string).toContain("🔴 待完成");
      expect(row as string).toContain("🟢 已完成");
