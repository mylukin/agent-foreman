...
| `analyze <spec_path>` | Analyze a requirement spec and generate ordered JSON step files |
| `run <steps_dir>` | Execute JSON steps generated by `analyze` |
...
- `run <steps_dir> --verify-generate-unittest` – Only check whether each step has `unit_test` configured and, if missing, call AI to generate `unit_test` info and write it back to the step JSON without changing business logic.
